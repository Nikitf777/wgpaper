...
fn main() -> Result<(), Box<dyn Error>> {
	...
	let result =
		spirv_builder::SpirvBuilder::new(shader_crate_dir, "spirv-unknown-vulkan1.1").build()?;
	...
	let spv_path = match result.module {
		...
		spirv_builder::ModuleResult::MultiModule(_modules) => {
			panic!(
				"Expected a single SPIR-V module, got multiple. \
